...
abstract class BetterPlayerControlsState<T extends StatefulWidget> extends State<T> {
  ...
  void _showQualitiesSelectionWidget() {
    ...
    if (children.isEmpty) {
      children.add(
        _buildTrackRow(BetterPlayerHlsTrack.defaultTrack(), betterPlayerController!.translations.qualityAuto),
      );
    }
  ...
  Widget _buildTrackRow(BetterPlayerHlsTrack track, String? preferredName) {
    final String trackName =
        preferredName ?? "${track.width}x${track.height} ${BetterPlayerUtils.formatBitrate(track.bitrate!)}";
  ...
  void _showAudioTracksSelectionWidget() {
    final List<BetterPlayerHlsAudioTrack>? tracks = betterPlayerController!.betterPlayerAudioTracks;
    final List<Widget> children = [];
    final BetterPlayerHlsAudioTrack? selectedAudioTrack = betterPlayerController!.betterPlayerAudioTrack;
    if (tracks != null) {
      for (var index = 0; index < tracks.length; index++) {
        final bool isSelected = selectedAudioTrack != null && selectedAudioTrack == tracks[index];
  ...
  Widget _buildAudioTrackRow(BetterPlayerHlsAudioTrack audioTrack, bool isSelected) {
    return BetterPlayerMaterialClickableWidget(
      onTap: () {
        Navigator.of(context).pop();
